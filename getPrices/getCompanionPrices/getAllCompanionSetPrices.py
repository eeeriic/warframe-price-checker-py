...
def getAllCompanionSetPrices():
    script_dir = Path(__file__).parent
    project_root = script_dir.parent.parent
    input_file = project_root / "filterItems" / "filtered_data.json"
    output_file = project_root / "prices" / "companions" / "companion_sets_prices.json"
    output_file.parent.mkdir(parents=True, exist_ok=True)
    
    with open(input_file, "r", encoding="utf-8") as file:
        data = json.load(file)["companions"]
        ...
        with open(output_file, "w", encoding="utf-8") as file:
            ...
            print("all companion prices saved")
